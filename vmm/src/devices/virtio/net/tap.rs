...
impl IfReqBuilder {
    ...
    pub fn new() -> Self {
        IfReqBuilder(ifreq {
            ...
            ifr_ifru: unsafe { std::mem::zeroed::<__c_anonymous_ifr_ifru>() },
        })
    }

    pub fn if_name(mut self, if_name: &[u8; IFACE_NAME_MAX_LEN]) -> Self {
        let ifrn_name = self.0.ifr_name.as_mut();
        for (dst, src) in ifrn_name.iter_mut().zip(if_name.iter()) {
            *dst = *src as i8;
        }
...
impl Tap {
    ...
    pub fn open_named(if_name: &str) -> Result<Tap> {
        ...
        for (dst, src) in if_name.iter_mut().zip(ifreq.ifr_name.iter()) {
            *dst = *src as u8;
        }
