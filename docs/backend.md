...
- `src/main.rs`: Entry point for the backend, initializes the HTTP server and routes.
- `src/initramfs_manager.rs`: Manages initramfs files for different languages and versions.
- `src/ip_manager.rs`: Handles IP address allocation for VMs.
- `src/vm_lifecycle.rs`: Manages the lifecycle of virtual machines, including creation and cleanup.
- `src/lib.rs`: Shared utilities and constants used across the backend.